...
public final class Calibrations {
    ...
    /**
     * Controller on module speed for rotating to target, input degrees [-180,180],
     * output [0,1].
     */
    public static final PIDController ROTATE_TO_TARGET_PID_CONTROLLER = new PIDController(0.030, 0, 0.000); // From 2022
    /**
     * Feed forward for rotating to target, gets added to or subtracted from PID
     * controller.
     */
    ...
    public static final double MAX_CHARGE_STATION_CLIMB_NORM_SPEED = MAX_CHARGE_STATION_CLIMB_SPEED_MPS
            / Constants.SwerveDrive.MAX_SPEED_METERS_PER_SECOND;
    /**
     * Conversion factor between the robot pitch (in degrees) to a velocity (in
     * [-1,1]) for balancing
     * on the charge station
     */
    public static final double CHARGE_STATION_PITCH_DEGREES_TO_NORM_VELOCITY = -1 * Constants.MAX_PITCH_DEGREES
            / MAX_CHARGE_STATION_CLIMB_NORM_SPEED;
    /**
     * Maximum speed to deadband at (that is to say, it won't auto-balance if the
     * velocity is <= 0.05,
